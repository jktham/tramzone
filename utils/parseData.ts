...
function getDate() {
  ...
  if (
    monday.getDate() == updateTime.getDate() &&
    monday.getTime() < updateTime.getTime()
  ) {
    monday.setDate(monday.getDate() - 7);
  }
  if (
    thursday.getDate() == updateTime.getDate() &&
    thursday.getTime() < updateTime.getTime()
  ) {
...
async function parseGtfs() {
  ...
  let filteredWriteStream = createWriteStream(
    "data/parsed/stopTimesFiltered.csv"
  );
...
async function parseLines() {
  ...
  let lineColors = JSON.parse(
    (await fs.readFile(`data/datasets/lineColors.json`)).toString()
  );
...
async function generateTramTrips() {
  ...
  let routes: Route[] = JSON.parse(
    (await fs.readFile("data/parsed/routes.json")).toString()
  );
  let trips: Trip[] = JSON.parse(
    (await fs.readFile("data/parsed/trips.json")).toString()
  );
  let stopTimes: StopTime[] = JSON.parse(
    (await fs.readFile("data/parsed/stopTimes.json")).toString()
  );
  let services: Service[] = JSON.parse(
    (await fs.readFile("data/parsed/services.json")).toString()
  );
  ...
  stopTimesMap.forEach((v, k) => {
    stopTimesMap.set(
      k,
      v.sort((a, b) => a.stop_sequence - b.stop_sequence)
    );
  });
  ...
  for (let i = 0; i < 7; i++) {
    ...
    fs.writeFile(
      `data/parsed/tramTrips${i}.json`,
      JSON.stringify(tramTrips_day)
    );
  }
}
