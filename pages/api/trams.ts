...
export default async function handler(
  ...
  let tramTrips: TramTrip[] = JSON.parse(
    (await fs.readFile(`data/parsed/tramTrips${weekday}.json`)).toString()
  ); // todo: next day
  ...
  let tripUpdates: TripUpdate[] = realtime["Entity"]
    .filter((e) => tripIds.has(e["Id"]))
    .map((t) => {
      return {
  ...
  let stations: Station[] = JSON.parse(
    (await fs.readFile("data/parsed/stations.json")).toString()
  );
    ...
    return {
      ...
      stops: t.stops.map((s) => {
        ...
        return {
          ...
          arrival_delay:
            update?.stops?.find((us) => us.stop_id == s.stop_id)
              ?.arrival_delay || 0,
          departure_delay:
            update?.stops?.find((us) => us.stop_id == s.stop_id)
              ?.departure_delay || 0,
  ...
  trams = trams.map((t) => {
    ...
    let prev_stop = t.stops.find(
      (s) => s.stop_sequence == Math.floor(t.progress)
    );
    let next_stop = t.stops.find(
      (s) => s.stop_sequence == Math.floor(t.progress + 1)
    );
  ...
  if (query.station) {
    trams = trams.filter((t) =>
      new Set(t.stops.map((s) => s.stop_diva)).has(query.station)
    );
  }
